...
velocity_list = [0] # Start velocityocity list.
...
y = 0				#0
z = 0				#0
rad = 6.371e6		#This is the radius
col1=color.blue  	#Earth is also known as the blue planet
body_list.append(sphere(radius = rad, 
                      position = (x,y,z),
                      color=col1,			#Allotting colour
                      make_trail = True))		#Change this to make the trail visible 
					  
					  
angular_momentum = 149.6e9**2 * 2*3.14159 * mass_list[2] / (365.25*24*3600)
vx = 0
vy = angular_momentum/(147.1e9*mass_list[2])
vz = 0
velocity_list.append(vector(vx,vy,vz))
...
obj = 1
while obj <= xyz:				
    force_list.append(vector(0,0,0))		#initialization
    obj = obj + 1
...
while True: # Begin motion loop.
    ...
    rate(5000)		#Randomly chosen from example problems
    ...
    while obj <=xyz:
        ...
        obj2 = obj + 1
        while obj2 <= xyz:
            r = body_list[obj].position - body_list[obj2].position
        ...
        body_list[obj].position = body_list[obj].position + velocity_list[obj]*dt # Update body obj position.
        obj = obj + 1
